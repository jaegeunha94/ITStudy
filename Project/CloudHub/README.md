...
* 변경 이유: Telegraf의 Input Plugin를 테스트 하고 싶을 경우 전체 플러그인을 Test 하여 많은 시간이 소요됨 
* 변경 내용: Telegraf Plugin Test 화면 추가
...
* 변경 이유: Telegraf conf 파일이 변경되지 않았음에도, 파일을 덮어쓰는 필요없는 API 요청이 들어가 시간이 지연됨
* 변경 내용: telegraf conf 파일 변경되지 않았을 때 telegraf conf 파일 덮어쓰는 동작 막는 로직 추가
...
* 변경 이유: Topology 내용을 다른 서버에 옮기고 싶을 때, 많은 시간이 소요됨
* 변경 내용: Topology 내용을 파일로 export/import하는 UI 및 기능 추가
...
* 변경 이유
  * AgentConfiguration 화면에서 설정 파일을 적용할 경우 서비스를 restart 시키는 문제
* 변경 내용
  * AgentConfiguration 화면에서 설정 파일을 적용할 경우 서비스를 restart 시키는 것에서 reload로 변경
...
* 변경 이유: 설정 파일을 적용하기 전에 테스트 로직을 실행하지 않으면, 잘못된 설정 파일이 적용됐을 경우 Telegraf 서비스 파일이 죽을 수 있음
* 변경 내용: Cloud Input Plugin 적용 전 Telegraf 테스트 로직 후 성공 시 Conf 파일 적용하는 로직 추가
...
* 변경 이유: Telegraf Test 파일명에 TimeStamp를 추가하기 위함
* 변경 내용: ServiceConfig 탭의 Telegraf Test File 명 변경
...
* 변경 이유: API 요청시 상태값이 200인 경우 성공한 것으로 보기 위해 로직 수정
* 변경 내용: NGinx 적용 후 변경된 response status에 맞게 분기문 변경
...
* 변경 내용: AgentConfiguration 탭과 ServiceConfig 탭에 Collector Server를 HostName으로 필터링하는 기능 추가
...
* 변경 이유: Agent Minions 탭에서 Console을 클릭할 때, 선택한 Host에서 사용하는 네트워크 인터페이스가 2개 이상일 경우 선택할 수 없음 
* 변경 내용: Agent Minions 탭에서 Conosle을 클릭할 때 IP 선택할 수 있는 UI 추가
...
* 변경 이유: API 요청시 Frontend 코드에서 token을 넣어서 보내기 때문에 개발자도구 Network 탭에서 token이 노출됨
* 변경 내용
...
* 변경 이유: Salt API 사용 구간에서 개발자 인증서를 사용하기 때문에 인증서 Verify 작업을 Skip하는 로직이 필요 
...
* 변경 내용: 브라우저에서 제공하는 confirm 기능을 사용하여 페이지 전환 시, 변경 사항을 저장할 지 물어보는 것으로 결정
